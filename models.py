...
class ConductorDB(Base):
    ...
    licencia = Column(String(20), unique=True, nullable=False)
    experiencia_anos = Column(Integer, default=0)
    vehiculo_actual_id = Column(Integer, ForeignKey('vehiculos.id'), nullable=True)

class RutaDB(Base):
    __tablename__ = 'rutas'
    id = Column(Integer, primary_key=True, autoincrement=True)
    nombre = Column(String(100), unique=True, nullable=False)
    codigo_ruta = Column(String(10), unique=True, nullable=False)
...
class RutaParadaDB(Base):
    ...
    orden_secuencia = Column(Integer, nullable=False)
...
class UsuarioDB(Base):
    ...
    tipo_usuario = Column(String(20), default="Regular")

class HistorialViajesDB(Base):
    __tablename__ = 'historial_viajes'
    id = Column(Integer, primary_key=True, autoincrement=True)
    usuario_id = Column(String(20), ForeignKey('usuarios.id'), nullable=False)
    ruta_id = Column(Integer, ForeignKey('rutas.id'), nullable=False)
    vehiculo_id = Column(Integer, ForeignKey('vehiculos.id'), nullable=False)
    fecha = Column(DateTime, default=datetime.datetime.utcnow)
    costo_aplicado = Column(Float, nullable=False)
    metodo_pago = Column(String(20), default="Tarjeta_NFC")
